...
rows = [
    # A1
    ("A1", "Outliers",
     "Min. = -1.26 and max. = 60 against a 3rd Qrt. of only 0.69, so both extremes lie far from centre.",
     "Clamp the instances",
    ...
    #  A2
    ("A2", "Cardinality",
     "Card. = 133 distinct levels, the highest of any categorical feature (every other has 12 or less); the two modes tcp (47.75%) and udp (35.98%) already cover about 84%, leaving a long tail of roughly 131 rare levels.",
    ...
    #  A5
    ("A5", "Skewness",
     "Strong right skew: mean 19.8 vs. median 4, with values trailing out to max. 10,646.",
     "Apply log transformation to the feature",
     "A long right tail pulls the summary stats away from the bulk of the data. A logarithm reshapes the distribution toward symmetry so the typical values are no longer compressed against the axis."),
    ("A5", "Correlation",
    ...
    #  A6
    ("A6", "Skewness",
     "Right-skewed: mean 18.5 vs. median 2, max. 11,018.",
     "Apply log transformation to the feature",
     "A long right tail pulls the summary stats away from the bulk of the data. A logarithm reshapes the distribution toward symmetry so the typical values are no longer compressed against the axis."),
    ("A6", "Correlation",
    ...
    ("A7", "Independent",
     ...
     "Remove the feature",
     "A feature that has a unique value for every row but has no relationship with the target variable effectively behaves like random noise. It provides no useful information for prediction and may cause the model to learn patterns that are not actually meaningful."),

    #  A8
    ("A8", "Skewness",
     "Right-skewed: mean 8,573 vs. median 528, max. 14,355,774.",
     "Apply log transformation to the feature",
    ...
    #  A9
    ("A9", "Skewness",
     "Right-skewed: mean 14,387 vs. median 178, max. 14,657,531.",
     "Apply log transformation to the feature",
    ...
    ("A11", "Cardinality",
     ...
     "Bin the feature into intervals",
     "A numeric feature with very few distinct values is better treated as an ordinal variable than as a continuous one. Grouping the values into intervals reflects this structure and avoids implying a level of precision that the data does not support."),
    ...
    #  A13
    ("A13", "Magnitude",
     "Values span about 1.2e24 to 5.99e29 with mean 7.06e27, making this feature several orders of magnitude larger than all the other features.",
     "Apply log transformation to the feature",
    ...
    ("A20", "Outliers",
     ...
     "Clamp the instances",
     "The maximum value is much larger than the interquartile range, indicating the presence of clear high-value outliers. Clamping these extreme values prevents them from disproportionately affecting the scale of the feature."),

    #  A21
    ("A21", "Skewness",
     "Right-skewed: mean 5,419 against median 0.67, max. 1,483,831.",
     "Apply log transformation to the feature",
     "Since most of the values are very small compared to the maximum, applying a logarithmic transformation helps reveal patterns among the smaller values without allowing the extreme upper tail to dominate the scale."),

    #  A22
    ("A22", "Missing Values",
     "% Miss. = 0.51 (Med. = 0); the identical rate seen in other features suggests a shared set of missing rows.",
     "Impute the values with the median",
    ...
    #  A23
    ("A23", "Correlation",
     "The correlation with A26 is 0.98 (a very strong positive corrl.), and the two hold identical values on 99.01% of rows.",
     "Remove one of the features",
    ...
    #  A25
    ("A25", "Missing Values",
     "% Miss. = 0.51 on a large-scale feature (vals up to about 4.3e9); Median = 0.",
     "Impute the values with the median",
    ...
    # A28
    ("A28", "Missing Values",
     "% Miss. = 0.51 (Med. = 0), matching the shared missingness rate across features.",
     "Impute the values with the median",
    ...
    # A32
    ("A32", "Missing Values",
     "% Miss. = 0.51 (Med. = 0, values almost all zero).",
     "Impute the values with the median",
    ...
    ("A33", "Outliers",
     ...
     "Clamp the instances",
     "Most of the values are zero, with a small number of extremely large values creating a clear outlier pattern. Capping these extreme values prevents them from disproportionately affecting the feature's scale."),
    ...
    ("A40", "No information",
     ...
     "Remove the feature",
     "The values represent the row index rather than an actual measured feature. They only indicate the position of each record in the dataset and therefore do not provide useful information that can generalise to unseen data."),
    ...
    # A43
    ("A43", "Cardinality",
     "Card. = 11, with almost all values 0 (Med. = 3rd Qrt. = 0) and max. = 30.",
     "Bin the feature into intervals",
...
# ---------------------------------------------------------------------------
# Correlation rows: ONE row per feature (spec: "create multiple rows for
# features with the same data quality issue"; the Feature column names a single
# column). Every continuous feature that takes part in a very strong
# correlation (|r| close to +1, i.e. >= 0.90 per the diagnostic run) gets its
# own row listing its correlate(s). This mirrors how every other issue type is
# recorded one-per-feature. A <-> B is symmetric, so both endpoints appear.
# The pairs and r values come from diagnose_task2.py / check_completeness.py.
CORR = {
    "A5":  ["A15 (r = 0.97)", "A8 (r = 0.96)"],
    "A6":  ["A16 (r = 0.98)", "A9 (r = 0.97)"],
    "A8":  ["A15 (r = 0.996)", "A5 (r = 0.96)"],
    "A9":  ["A16 (r = 0.997)", "A6 (r = 0.97)"],
    "A15": ["A8 (r = 0.996)", "A5 (r = 0.97)"],
    "A16": ["A9 (r = 0.997)", "A6 (r = 0.98)"],
    "A23": ["A26 (r = 0.98)"],
    "A26": ["A23 (r = 0.98)"],
    "A27": ["A28 (r = 0.94)", "A29 (r = 0.92)"],
    "A28": ["A27 (r = 0.94)"],
    "A29": ["A27 (r = 0.92)"],
    "A34": ["A45 (r = 0.98)", "A39 (r = 0.95)"],
    "A36": ["A37 (r = 0.96)", "A44 (r = 0.90)"],
    "A37": ["A36 (r = 0.96)", "A44 (r = 0.91)", "A38 (r = 0.91)"],
    "A38": ["A37 (r = 0.91)"],
    "A39": ["A45 (r = 0.96)", "A34 (r = 0.95)"],
    "A44": ["A37 (r = 0.91)", "A36 (r = 0.90)"],
    "A45": ["A34 (r = 0.98)", "A39 (r = 0.96)"],
}


def _corr_row(feat, partners):
    ev = (f"{feat} is very strongly correlated (r close to +1) with "
          + ", ".join(partners)
          + ", so it carries essentially the same variation as those feature(s).")
    ju = ("This feature belongs to a correlated group that repeats the same "
          "information; only one representative of the group needs to be kept, "
          "so this one is redundant and can be dropped.")
    return (feat, "Correlation", ev, "Remove one of the features", ju)


# Drop any hand-written per-pair Correlation rows above and replace them with
# the generated one-per-feature set, then keep the table in feature order.
rows = [r for r in rows if r[1] != "Correlation" or r[0] == "A42"]
rows += [_corr_row(f, p) for f, p in CORR.items()]


def _feat_key(r):
    f = r[0]
    return (0, int(f[1:])) if f[1:].isdigit() else (1, 0)  # A1..A46 then T


rows.sort(key=_feat_key)  # stable: each feature keeps its rows' relative order
